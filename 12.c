#include "a.h"
#define N 1000
I dp[N][N],i,m,j,k,n,sx,sy,ex,ey=-1,dx[]={0,0,-1,1},dy[]={1,-1,0,0}; B b[N][N],h=1,t;
TD S{I y,x;}P;P q[N*N];
V BFS(V) {
    W(h-t,P p=q[t++]; LP(i,4,P np={p.y+dy[i],p.x+dx[i]};
            I(np.y<0||np.y>=n||np.x<0||np.x>=m||b[np.y][np.x]=='#',CT)
            I(b[np.y][np.x]-b[p.y][p.x]>1,CT)
            I(dp[np.y][np.x]>dp[p.y][p.x]+1,dp[np.y][np.x]=dp[p.y][p.x]+1,q[h++]=np)
        ))
}
I main(V) {
